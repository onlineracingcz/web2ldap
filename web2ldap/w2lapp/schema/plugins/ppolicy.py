...
import time,datetime,utctime,w2lapp.searchform
...
class PwdMaxAge(Timespan):
  ...
  def _timespan_search_params(self):
    return (
      ...
      ('search_option',w2lapp.searchform.SEARCH_OPT_LE_THAN),
...
class PwdExpireWarning(PwdMaxAge):
  ...
  def _timespan_search_params(self):
    ...
    return (
      ('search_attr','pwdChangedTime'),
      ('search_option',w2lapp.searchform.SEARCH_OPT_GE_THAN),
    ...
    try:
      ...
      if pwd_max_age_secs:
        ...
        expire_msg = '%s %s (%s %s)' % (
          ...
          self._form.utf2display(w2lapp.gui.ts2repr(self.time_divisors,u' ',abs(expired_since))),
          {
            -1:'ago',
            0:'',
            1:'ahead',
