# -*- coding: utf-8 -*-
"""
w2lapp.search: do a search and return results in several formats
...
class PrintableHTMLWriter(ldaputil.async.List):
  ...
  def processResults(self,ignoreResultsNumber=0,processResultsCount=0,timeout=-1):
    ...
    print_cols = w2lapp.cnf.GetParam(self._ls,'print_cols','4')
    table=[]
        ...
        if template_oc:
          ...
          for attr in attr_list:
            tableentry[attr] = ', '.join([
              utf2display(unicode(attr_value,self._ls.charset))
    ...
    w2lapp.gui.TopSection(self._sid,self._outf,'search',self._form,self._ls,self._dn,'Printable Search Results',[])
    self._outf.write("""
<table
...
""")
    ...
    w2lapp.gui.Footer(self._outf,self._form)
...
def w2l_Search(sid,outf,command,form,ls,dn,connLDAPUrl):
  ...
  try:
    ...
    w2lapp.searchform.w2l_SearchForm(
      sid,outf,command,form,ls,dn,
      Msg=' '.join((
        w2lapp.gui.LDAPError2ErrMsg(e,form,charset=ls.charset),
    ...
    try:
      ...
      if SearchNoOpControl.controlType in ls.supportedControl:
        try:
          ...
          if num_all_search_results!=None and num_all_search_continuations!=None:
    ...
    if not result_dnlist:
      ...
      if (partial_results and search_size_limit>0) or search_resminindex:
        ...
        if partial_results:
          ...
          if num_result_all!=None and resind<num_result_all:
          ...
          if search_output=='raw':
            ...
            if tdtemplate_oc:
              ...
              if tableentry_attrs:
                ...
                tableentry = w2lapp.read.DisplayEntry(sid,form,ls,dn,sub_schema,ldaputil.schema.Entry(sub_schema,dn,entry),'searchSep',0)
                tdlist = []
                for oc in tdtemplate_oc:
      ...
      """ % (
        '\n'.join((
          ...
          w2lapp.form.InclOpAttrsCheckbox(
            'search_opattrs',
