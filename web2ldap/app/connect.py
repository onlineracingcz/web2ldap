...
import types,time,pyweblib.forms,web2ldap.app.core,web2ldap.app.gui,web2ldap.app.cnf
...
def w2l_Connect(outf,form,env,Msg='Connect',ErrorMsg=''):
  ...
  connect_template_str = web2ldap.app.gui.ReadTemplate(
    ...
    tmpl_filename=web2ldap.app.cnf.misc.connect_template
  )

  if web2ldap.app.cnf.hosts.ldap_uri_list:
    uri_select_list = []
    for uri in web2ldap.app.cnf.hosts.ldap_uri_list:
      if type(uri)==types.TupleType:
        uri,description = uri
      else:
        description = web2ldap.app.cnf.ldap_def.get(
          uri,
          web2ldap.app.cnf.hosts.Web2LDAPConfig()
  ...
  outf.write(connect_template_str.format(
    ...
    disable_start={0:'',1:'<!--'}[web2ldap.app.cnf.hosts.restricted_ldap_uri_list],
    disable_end={0:'',1:'-->'}[web2ldap.app.cnf.hosts.restricted_ldap_uri_list],
