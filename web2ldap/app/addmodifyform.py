...
class InputFormEntry(web2ldap.app.read.DisplayEntry):
    ...
    def __init__(
            self, sid, form, command, ls, dn, schema, entry,
            writeable_attr_oids,
        ...
        web2ldap.app.read.DisplayEntry.__init__(
            self, sid, form, ls, dn, schema, entry, 'fieldSep', False
        )
    ...
    def __getitem__(self, nameoroid):
        ...
        attr_inst = syntax_class(
            self.sid, self.form, self.ls, self.dn, self.entry._s, nameoroid, None, self.entry
        )
        ...
        for attr_index, attr_value in enumerate(attr_values):
            ...
            attr_inst = syntax_class(
                self.sid, self.form, self.ls, self.dn, self.entry._s, nameoroid, attr_value, self.entry,
            )
                ...
                for input_field in input_fields:
                    ...
                    result.append('\n'.join([
                        ...
                        input_field.inputHTML(
                            ...
                            )).encode(self.form.accept_charset),
                            title=attr_title
                        ),
                        attr_inst.valueButton(self.command, self.row_counter, '+'),
                        attr_inst.valueButton(self.command, self.row_counter, '-'),
...
def SupentryDisplayString(
    ...
    if inputform_supentrytemplate:
        ...
        try:
            parent_search_result = ls.readEntry(
                parent_dn,
...
def ObjectClassForm(
    ...
    def get_possible_soc(ls, sub_schema, parent_dn):
        ...
        # attribute 'allowedChildClasses' (MS AD or OpenLDAP with slapo-allowed)
                ...
                'OpenLDAProotDSE' not in ls.rootDSE.get('objectClass', [])
            ):
            try:
                parent_search_result = ls.readEntry(
                    parent_dn,
    ...
    def ExpertOCFields(ls, sub_schema, parent_dn):
        ...
        if existing_object_classes and structural_object_class:
            ...
            if dit_content_rule is not None:
                ...
                dit_content_rule_html = '%s<br>DIT content rule:<br>%s' % (
                    ...
                    web2ldap.app.gui.SchemaElementName(sid, form, dn, sub_schema, dit_content_rule.names[0], DITContentRule)
                )
        ...
        input_currentformtype = form.getInputValue('in_oft', ['Template'])[0]
    ...
    def LDIFTemplateField(ls, form, parent_dn):
        ...
        for template_name in addform_entry_templates_keys:
            ...
            if addform_parent_attrs:
                try:
                    parent_result = ls.readEntry(tmpl_parent_dn, attrtype_list=addform_parent_attrs)
        ...
        for pdn in sorted(add_tmpl_dict.keys()):
            ...
            for tmpl_name in add_tmpl_dict[pdn]:
                add_template_html_list.append(
                    '<li>%s</li>' % (
                        form.applAnchor(
                            'add', form.utf2display(tmpl_name), sid,
                            [
                                ('dn', pdn),
    ...
    if command == 'add':
        context_menu_list = [
            form.applAnchor('add', 'Templates', sid, [('dn', dn), ('in_ocf', 'tmpl')]),
            form.applAnchor('add', 'Expert', sid, [('dn', dn), ('in_ocf', 'exp')]),
        ]
    else:
...
def w2l_modifyform(sid, outf, command, form, ls, dn, entry, Msg='', invalid_attrs=None):
    ...
    input_formtype = form.getInputValue(
        'in_ft',
        form.getInputValue('in_oft', ['Template'])
    )[0]
    ...
    input_form_entry = InputFormEntry(
        sid, form, command, ls, dn, sub_schema,
    ...
    outf.write(
        INPUT_FORM_BEGIN_TMPL.format(
            ...
            form_begin=form.beginFormHTML(command, sid, 'POST', target=None, enctype='multipart/form-data'),
            field_dn=form.hiddenFieldHTML('dn', dn, u''),
            field_currentformtype=form.hiddenFieldHTML('in_oft', input_formtype.decode('ascii'), u''),
            text_submit=command[0].upper()+command[1:],
        )
    )
