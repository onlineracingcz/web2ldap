...
class SshPublicKey(DirectoryString):
  ...
  reObj = re.compile('(^|.* )(ssh-rsa|ssh-dss|ecdsa-sha2-nistp256|ecdsa-sha2-nistp384|ecdsa-sha2-nistp521|ssh-ed25519) (?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?(| .+)$')
  ...
  def _strip_padding(self, b64_val):
    i = len(b64_val)
    while b64_val[i-1]=='=':
      i = i-1
    return b64_val[:i]
  ...
  def _display_lines(self,valueindex,commandbutton,pk_type,pk_comment,pk_bin,pk_fingerprints):
    ...
    if pk_fingerprints:
      ...
      for hash_algo,pk_fingerprint in sorted(pk_fingerprints.items()):
        result.append('<dt>{0}:</dt><dd>{1}</dd>'.format(
            hash_algo.upper(),
...
class ParamikoSshPublicKey(SshPublicKey):
    ...
    def _get_key_size(self,pk_type,pk_bin):
      try:
