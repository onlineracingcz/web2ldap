...
def display_nameoroid(sid, form, dn, schema, se_nameoroid, se_class):
    se = schema.get_obj(se_class, se_nameoroid, default=None)
    if se is None:
        return se_nameoroid
    return form.applAnchor(
        'oid',
        link_text(se, form.accept_charset),
        sid,
        [
            ('dn', dn),
            ('oid', se.oid),
...
def display_nameoroid_list(sid, form, dn, schema, se_names, se_class):
    link_texts = []
    for se_nameoroid in se_names:
        se = schema.get_obj(se_class, se_nameoroid, default=None)
        if se:
            ltxt = link_text(se, form.accept_charset)
            try:
                schema_id = se.oid
    ...
    try:
        ...
        if subschemaSubentry:
            ...
            for se_class_attr in se_class_attrs:
                context_menu_list.append(
                    form.applAnchor(
                        'oid', se_class_attr, sid,
                        [('dn', dn), ('oid_class', se_class_attr)],
...
class DisplaySchemaElement:
    ...
    def disp_details(self, sid, outf, form, dn):
        ...
        for text, class_attr, se_class in self.detail_attrs:
            ...
            if class_attr_value is None:
                ...
                if se_class is None:
                    value_output = ', '.join([
                        form.utf2display(v.decode('utf-8'), sp_entity=' ', lf_entity='<br>')
    ...
    def display(self, sid, outf, form, ls, dn):
        ...
        web2ldap.app.gui.TopSection(
            ...
            context_menu_list=schema_context_menu(sid, form, ls, dn)
        )
        outf.write(
            """
            %s
    ...
    def display(self, sid, outf, form, ls, dn):
        ...
        outf.write(
            ...
            """ % (
                ...
                self.se.ruleid,
                form.utf2display(str(self.se).decode(ls.charset)),
                ''.join(self.disp_details(sid, outf, form, dn)),
            )
        )
        web2ldap.app.gui.Footer(outf, form)
...
def oid_input_form(form, sid, dn, oid=None):
...
def display_schema_elements(sid, outf, form, ls, dn, schema, se_classes, se_list):
    se_list = se_list or []
    ...
    outf.write(oid_input_form(form, sid, dn, ''))
    ...
    if oid_dict:
        for schema_class in oid_dict.keys():
            ...
            outf.write('<h2>%s</h2>\n<p>found %d</p>\n%s\n' % (
                ...
                len(schema_elements),
                ',\n '.join(display_nameoroid_list(sid, form, dn, schema, schema_elements, schema_class)),
            ))
    else:
        outf.write(SCHEMA_VIEWER_USAGE)
    web2ldap.app.gui.Footer(outf, form)
    ...
    if not se_list:
        ...
        web2ldap.app.gui.SimpleMessage(
            ...
            message='<h1>Schema elements</h1><p class="ErrorMessage">Name or OID not found in schema!</p><p>%s</p>' % (
                oid_input_form(form, sid, dn, oid)
            ),
            main_div_id='Message',
        ...
        display_schema_elements(sid, outf, form, ls, dn, sub_schema, None, se_list)
        return
