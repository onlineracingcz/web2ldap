# -*- coding: utf-8 -*-
"""
w2lapp.read: Read single entry and output as HTML or vCard
...
class DisplayEntry(ldaputil.schema.Entry):
  ...
  def __getitem__(self,nameoroid):
    try:
      ...
      return ''
    result = []
    syntax_se = w2lapp.schema.syntaxes.syntax_registry.syntaxClass(self._s,nameoroid,self.structuralObjectClass)
  ...
  def get_html_templates(self,cnf_key):
    read_template_dict = cidict(w2lapp.cnf.GetParam(self.ls,cnf_key,{}))
  ...
  def template_output(self,outf,cnf_key,display_duplicate_attrs=True):
    ...
    structural_oc,abstract_oc,auxiliary_oc = w2lapp.schema.object_class_categories(self._s,template_oc)
    ...
    for oc in template_oc:
      try:
        ...
        read_template_filename = w2lapp.gui.GetVariantFilename(read_template_filename,self.form.accept_language)
...
def w2l_Read(
  ...
  if len(wanted_attrs)==1 and \
    ...
    syntax_se = w2lapp.schema.syntaxes.syntax_registry.syntaxClass(sub_schema,attr_type,entry.get_structural_oc())
  ...
  if read_output in (u'table',u'template'):
    ...
    if vcard_template_filename:
      ...
      try:
        ...
        raise w2lapp.core.ErrorExit(u'I/O error during reading vCard template file!')
      else:
