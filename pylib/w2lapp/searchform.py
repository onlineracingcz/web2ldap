# -*- coding: utf-8 -*-
"""
w2lapp.searchform: different search forms
...
def SearchForm_adv(form,ls,dn,sub_schema):
  ...
  if search_submit.startswith('-'):
    ...
    if len(search_attr_list)<w2lapp.cnf.misc.max_searchparams:
  ...
  search_attr_select = w2lapp.gui.AttributeTypeSelectField(
    form,ls,sub_schema,
    'search_attr',
    u'Search attribute type',
    search_attr_list,
    default_attr_options=w2lapp.cnf.GetParam(ls,'search_attrs',[])
  )
...
def w2l_SearchForm(
  ...
  naming_contexts = w2lapp.cnf.GetParam(ls,'naming_contexts',None)

  search_root = form.getInputValue('search_root',[search_root or ls.getSearchRoot(dn,naming_contexts)])[0]
  search_root_field = w2lapp.gui.SearchRootField(
    form,ls,dn,
    name='search_root',
    search_root_searchurl=w2lapp.cnf.GetParam(ls,'searchform_search_root_url',None),
  ...
  """.format(
    ...
    searchoptions_template_str=searchoptions_template_str.format(
      ...
      field_search_resnumber=form.field['search_resnumber'].inputHTML(
        default=unicode(w2lapp.cnf.GetParam(ls,'search_resultsperpage',10))
      ),
