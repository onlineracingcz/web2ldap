...
def w2l_Add(sid,outf,command,form,ls,dn):
  ...
  if add_clonedn:
    ...
    add_dn,entry = w2lapp.addmodifyform.ReadLDIFTemplate(ls,form,add_template)
    entry = ldaputil.schema.Entry(sub_schema,None,entry)
    add_rdn,add_basedn = ldaputil.base.SplitRDN(add_dn.decode(ls.charset))
    add_basedn = add_basedn or dn

  else:
    entry,invalid_attrs = w2lapp.modify.GetEntryfromInputForm(form,ls,dn,sub_schema)
  ...
  try:
    ...
    w2lapp.gui.SimpleMessage(
      ...
      """ % (
        ...
        w2lapp.gui.DisplayDN(sid,form,ls,new_dn_u,commandbutton=0),
