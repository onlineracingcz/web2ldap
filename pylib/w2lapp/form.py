# -*- coding: utf-8 -*-
"""
w2lapp.form: class for web2ldap input form handling
...
class Web2LDAPForm(pyweblib.forms.Form):
  ...
  cookie_length = w2lapp.cnf.misc.cookie_length
  cookie_max_age = w2lapp.cnf.misc.cookie_max_age
  cookie_domain = w2lapp.cnf.misc.cookie_domain
  ...
  def outFileObject(self,outf):
    ...
    """
    outfile = outf
    if w2lapp.cnf.misc.gzip_level>0 and \
  ...
  def addGeneralFields(self):
    self.addField(pyweblib.forms.Input(
      ...
      w2lapp.session.session.session_id_len,
      1,
      w2lapp.session.session.session_id_re.pattern
    ))
    ...
    self.addField(pyweblib.forms.Input('host',u'Host:Port',255,1,'(%s|[a-zA-Z0-9/._-]+)' % w2lapp.gui.host_pattern,size=30))
    self.addField(DistinguishedNameInput('dn','Distinguished Name'))
    self.addField(pyweblib.forms.Select(
      'scope','Scope',1,
      options=w2lapp.searchform.SEARCH_SCOPE_OPTIONS,
      default=w2lapp.searchform.SEARCH_SCOPE_STR_SUBTREE),
    )
  ...
  def hiddenFieldHTML(self,name,value,desc):
    return w2lapp.gui.HIDDEN_FIELD % (
      name,
...
class Web2LDAPForm_searchform(Web2LDAPForm):
  ...
  def addCommandFields(self):
    ...
    self.addField(pyweblib.forms.Input('searchform_template',u'Search form template name',60,w2lapp.cnf.misc.max_searchparams,u'[a-zA-Z0-9. ()_-]+'))
    self.addField(
    ...
    self.addField(pyweblib.forms.Input('search_attr',u'Attribute(s) to be searched',1000,w2lapp.cnf.misc.max_searchparams,ur'[\w,_;-]+'))
    self.addField(pyweblib.forms.Input('search_mr',u'Matching Rule',1000,w2lapp.cnf.misc.max_searchparams,ur'[\w,_;-]+'))
    self.addField(pyweblib.forms.Select('search_option',u'Search option',w2lapp.cnf.misc.max_searchparams,options=w2lapp.searchform.search_options))
    self.addField(pyweblib.forms.Input('search_string',u'Search string',100,w2lapp.cnf.misc.max_searchparams,u'.*',size=60))
...
class Web2LDAPForm_input(Web2LDAPForm):
  ...
  def addCommandFields(self):
    ...
    self.addField(AttributeType('in_at',u'Attribute type',w2lapp.cnf.misc.input_maxattrs))
    self.addField(AttributeType('in_avi',u'Value index',w2lapp.cnf.misc.input_maxattrs))
    self.addField(
      AttributeValueInput(
        'in_av',u'Attribute Value',
        w2lapp.cnf.misc.input_maxfieldlen,
...
class Web2LDAPForm_add(Web2LDAPForm_input):
  def addCommandFields(self):
    ...
    self.addField(pyweblib.forms.Input('add_template',u'LDIF template name',60,w2lapp.cnf.misc.max_searchparams,u'.+'))
...
class Web2LDAPForm_modify(Web2LDAPForm_input):
  def addCommandFields(self):
    Web2LDAPForm_input.addCommandFields(self)
    self.addField(AttributeType('in_oldattrtypes',u'Old attribute types',w2lapp.cnf.misc.input_maxattrs))
    self.addField(AttributeType('in_wrtattroids',u'Writeable attribute types',w2lapp.cnf.misc.input_maxattrs))
...
class Web2LDAPForm_bulkmod(Web2LDAPForm):
  def addCommandFields(self):
    ...
    self.addField(AttributeType('bulkmod_at',u'Attribute type',w2lapp.cnf.misc.input_maxattrs))
    self.addField(
      pyweblib.forms.Select(
        'bulkmod_op',
        u'Modification type',
  ...
  def addCommandFields(self):
    self.addField(
      pyweblib.forms.Select(
        ...
        u'Password action',
        1,
        options=[ (action,short_desc) for action,short_desc,_ in w2lapp.passwd.PASSWD_ACTIONS ],
    ...
    self.addField(pyweblib.forms.Select('passwd_scheme',u'Password hash scheme',1,options=w2lapp.passwd.available_hashtypes,default=w2lapp.passwd.available_hashtypes[-1]))
